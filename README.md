...
- **Ingress & Security:** PiVPN LXC running Wireguard acting as the exclusive cryptographic gateway.
- **Reverse Proxy & SSL:** Nginx Proxy Manager (LXC) handling local SSL termination with a trusted Let's Encrypt certificate via Cloudflare API DNS challenges.
- **Backend:** FastAPI (LXC) listening locally on `0.0.0.0`.
